...
VOID QueryWorkQueue(PWORKQUEUE WorkQueueThread)
{
		...
		Thread = GetNextProcessThread(Process, Thread,TRUE))
	{
			...
			MmIsAddressValidEx((PVOID)((ULONG)Thread+ulSuspendCount)))
		{
			...
			//获取系统的工作线程
			if (SuspendCount){
				ulText =(ULONG) ((ULONG)KernelStack + ulSizeOfSystemWorkSuspendThread);  //这里是硬编码，暂停线程的时候work入口地址偏移
			}else{
				ulText =(ULONG) ((ULONG)KernelStack + ulSizeOfSystemWorkThread);  //这里是硬编码，线程正常的时候work入口地址偏移
			}
		    ...
		    //线程暂停，这个时候大小就变了。
			if (SuspendCount){
				ulText =(ULONG) ((ULONG)KernelStack + ulSizeOfOtherWorkSuspendThread);  //这里是硬编码，暂停线程的时候work入口地址偏移
			}else
				ulText =(ULONG) ((ULONG)KernelStack + ulSizeOfOtherWorkThread);  //这里是硬编码，线程正常的时候work入口地址偏移
