...
BOOL SystemCallEntryShadowSSDTTableHook(char *FunctionName,int *Index,DWORD NewFuctionAddress)
{
	...
	__try
	{
		if (MmIsAddressValidEx(g_Safe_ServiceDescriptorShadowSSDTTable) &&
			MmIsAddressValidEx(g_Safe_ServiceDescriptorShadowSSDTTable->ServiceTable))
		{
			if (*Index >= 0 && *Index <(int) g_Safe_ServiceDescriptorShadowSSDTTable->TableSize)
			{
...
// 				}
				...
				InterlockedExchange(&g_Safe_ServiceDescriptorShadowSSDTTable->ServiceTable[*Index],NewFuctionAddress);
				bRetOK = TRUE;
...
BOOL SystemCallEntryShadowSSDTTableUnHook(int Index)
{
	...
	g_Safe_ServiceDescriptorShadowSSDTTable->ServiceTable[Index] = 0;
