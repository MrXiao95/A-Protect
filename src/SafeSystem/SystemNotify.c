...
*/
	 ...
	 if (!MmIsAddressValidEx((PVOID)Address)){
		 return ;
	 }
	Ref=(PEX_FAST_REF)(Address);
            
	CreateProcessNum = SystemNotify->ulCount;
	for(i=(PUCHAR)0;i<(PUCHAR)8;i++)
	{
...
void GetBugCheckCallBack(PSYSTEM_NOTIFY SystemNotify)
{
	...
	for(i=Base;i<i+SizeOfProc(Base);i++)
	{
	 ...
	 pBugCheckRecordPack=(PKBUGCHECK_CALLBACK_RECORD)Address;
	 pListEntry=&(pBugCheckRecordPack->Entry);
	 pListNext=pListEntry->Flink;
	 {
		pBugCheckRecordPack=(PKBUGCHECK_CALLBACK_RECORD)pListNext;
		...
		SystemNotify->NotifyInfo[BugCheckNum].ulObject =(ULONG) pBugCheckRecordPack;
		...
		if (!IsAddressInSystem(
			(ULONG)pBugCheckRecordPack->CallbackRoutine,
			&ulModuleBase,
			&ulModuleSize,
...
void GetFileSystemNotifyCallBack(PSYSTEM_NOTIFY SystemNotify)
{
	...
	GetListHeadAddr();
	if (!MmIsAddressValidEx((PVOID)IopFsNotifyChangeQueueHead)){
		return;
	}
	if (g_bDebugOn)
		KdPrint(("IopFsNotifyChangeQueueHead:%08x\r\n",IopFsNotifyChangeQueueHead));

	pNotificationPacket=(PNOTIFICATION_PACKET)IopFsNotifyChangeQueueHead;
	...
	Driver_Object_Temp=(PDRIVER_OBJECT)(*(ULONG*)((ULONG)(pListNext)+8));
	{
		if (!MmIsAddressValidEx((PVOID)((ULONG)(pListNext)+12))){
			return;
		}
...
void GetShutDownCallBack(PSYSTEM_NOTIFY SystemNotify)
{
	...
	if (g_bDebugOn)
	 ...
	 pShutDownPack=(PSHUTDOWN_PACKET)Address;
	 ...
	 pListNext=pListEntry->Flink;
	 {
		pShutDownPack=(PSHUTDOWN_PACKET)pListNext;
...
void GetLeaveSessionCallback(PSYSTEM_NOTIFY SystemNotify)
{
	...
	if (!MmIsAddressValidEx((PVOID)Address)){
	 ...
	 CallbackList=(PSEP_LOGON_SESSION_TERMINATED_NOTIFICATION)Address;
...
// 		IopFsNotifyChangeQueueHead=Addr;
		for (i=(PUCHAR)Addr;i < (PUCHAR)((ULONG)i+SizeOfProc((PVOID)Addr));i++)
		{
			if (*i == 0xbe)
			{
	...
	case WINDOWS_VERSION_7_7600_UP:
		for (i=(PUCHAR)Addr;i < (PUCHAR)((ULONG)i+SizeOfProc((PVOID)Addr));i++)
		{
			if (*i == 0xe8)
			{
				...
				if (MmIsAddressValidEx((PVOID)ulIoRegisterFsRegistrationChangeMountAware))
				{
					if (g_bDebugOn)
						KdPrint(("ulIoRegisterFsRegistrationChangeMountAware:%08x\r\n",ulIoRegisterFsRegistrationChangeMountAware));

					for (x=(PUCHAR)ulIoRegisterFsRegistrationChangeMountAware;x < (PUCHAR)((ULONG)x+SizeOfProc((PVOID)ulIoRegisterFsRegistrationChangeMountAware));x++)
					{
						if (*x == 0xbf)
						{
							if (g_bDebugOn)
...
VOID RemoveNotifyRoutine(ULONG ImageNotify,int Type)
{
	...
	case 1:
		...
		ReLoadNtosCALL((PVOID)(&g_fnRPsSetCreateProcessNotifyRoutine),L"PsSetCreateProcessNotifyRoutine",g_pOldSystemKernelModuleBase,(ULONG)g_pNewSystemKernelModuleBase);
		if (g_fnRPsSetCreateProcessNotifyRoutine)
		{
			g_fnRPsSetCreateProcessNotifyRoutine((PCREATE_PROCESS_NOTIFY_ROUTINE)ImageNotify,TRUE);
		}
		break;
	case 2:
		//CreateThread
		//PsRemoveCreateThreadNotifyRoutine(ImageNotify);
		ReLoadNtosCALL((PVOID)(&g_fnRPsRemoveCreateThreadNotifyRoutine),L"PsRemoveCreateThreadNotifyRoutine",g_pOldSystemKernelModuleBase,(ULONG)g_pNewSystemKernelModuleBase);
		if (g_fnRPsRemoveCreateThreadNotifyRoutine)
		{
			g_fnRPsRemoveCreateThreadNotifyRoutine((PCREATE_THREAD_NOTIFY_ROUTINE)ImageNotify);
		}
		break;
	case 3:
		//LoadImage
		//PsRemoveLoadImageNotifyRoutine(ImageNotify);
		ReLoadNtosCALL((PVOID)(&g_fnRPsRemoveLoadImageNotifyRoutine),L"PsRemoveLoadImageNotifyRoutine",g_pOldSystemKernelModuleBase,(ULONG)g_pNewSystemKernelModuleBase);
		if (g_fnRPsRemoveLoadImageNotifyRoutine)
		{
			g_fnRPsRemoveLoadImageNotifyRoutine((PLOAD_IMAGE_NOTIFY_ROUTINE)ImageNotify);
		}
		break;
	case 4:
		//BugCheckCallBack
		//KeDeregisterBugCheckCallback(ImageNotify);
		ReLoadNtosCALL((PVOID)(&g_fnRKeDeregisterBugCheckCallback),L"KeDeregisterBugCheckCallback",g_pOldSystemKernelModuleBase,(ULONG)g_pNewSystemKernelModuleBase);
		if (g_fnRKeDeregisterBugCheckCallback)
		{
			g_fnRKeDeregisterBugCheckCallback((PKBUGCHECK_CALLBACK_RECORD)ImageNotify);
		}
		break;
	case 5:
		//BugCheckReasonCallBack
		//KeDeregisterBugCheckReasonCallback(ImageNotify);
		ReLoadNtosCALL((PVOID)(&g_fnRKeDeregisterBugCheckReasonCallback),L"KeDeregisterBugCheckReasonCallback",g_pOldSystemKernelModuleBase,(ULONG)g_pNewSystemKernelModuleBase);
		if (g_fnRKeDeregisterBugCheckReasonCallback)
		{
			g_fnRKeDeregisterBugCheckReasonCallback((PKBUGCHECK_REASON_CALLBACK_RECORD)ImageNotify);
		}
		break;
	case 6:
		//ShutDown
		//IoUnregisterShutdownNotification(ImageNotify);
		ReLoadNtosCALL((PVOID)(&g_fnRIoUnregisterShutdownNotification),L"IoUnregisterShutdownNotification",g_pOldSystemKernelModuleBase,(ULONG)g_pNewSystemKernelModuleBase);
		if (g_fnRIoUnregisterShutdownNotification)
		{
			g_fnRIoUnregisterShutdownNotification((PDEVICE_OBJECT)ImageNotify);
		}
		break;
	case 7:
		//SeFileSystem
		//SeUnregisterLogonSessionTerminatedRoutine(ImageNotify);
		ReLoadNtosCALL((PVOID)(&g_fnRSeUnregisterLogonSessionTerminatedRoutine),L"SeUnregisterLogonSessionTerminatedRoutine",g_pOldSystemKernelModuleBase,(ULONG)g_pNewSystemKernelModuleBase);
		if (g_fnRSeUnregisterLogonSessionTerminatedRoutine)
		{
			g_fnRSeUnregisterLogonSessionTerminatedRoutine((PSE_LOGON_SESSION_TERMINATED_ROUTINE)ImageNotify);
		}
		break;
	case 8:
		//FsNotifyChange
