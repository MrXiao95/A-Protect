...
BOOL InitKernelThreadData()
{
	...
	ulPsCreateSystemThread = (PUCHAR)((DWORD)MmGetSystemRoutineAddress(&functionName));
	if (ulPsCreateSystemThread)
	{
		ulReloadPsCreateSystemThread = (ULONG)((ULONG)ulPsCreateSystemThread - g_pOldSystemKernelModuleBase + (ULONG)g_pNewSystemKernelModuleBase);

		//屌，已经被hook过了
		if (*ulPsCreateSystemThread == 0xE9)
		{
			//获取被hook的跳转地址
			ulPsCreateSystemThread = (PUCHAR)(*(PULONG)((ULONG)ulPsCreateSystemThread+1)+(ULONG)((ULONG)ulPsCreateSystemThread+5));
		}
		bRetOK = HookFunctionByHeaderAddress(
			ulReloadPsCreateSystemThread,
			(DWORD)ulPsCreateSystemThread,
