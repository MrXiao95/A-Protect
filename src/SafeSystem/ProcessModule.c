	...
	__try
	{
		...
		else if ( WinVer == WINDOWS_VERSION_VISTA_2008 || WinVer == WINDOWS_VERSION_7_7000 || WinVer == WINDOWS_VERSION_7_7600_UP) //win7||vista||2008
		{
			...
			if (0 != StartingVpn<<12)
			{
				...
				if (MmIsAddressValidEx((PVOID)Subsection))
				{
...
VOID EunmProcessModuleByVirtualMemory(ULONG EProcess,PDLLINFO PDll)
{
	...
	case WINDOWS_VERSION_7_7600_UP:
	case WINDOWS_VERSION_7_7000:
		ReLoadNtosCALL((PVOID)(&g_fnRZwQueryVirtualMemory),L"ZwQueryVirtualMemory",g_pOldSystemKernelModuleBase,(ULONG)g_pNewSystemKernelModuleBase); //win7导出了，可以直接用
		break;
	case WINDOWS_VERSION_2K3_SP1_SP2:
		if (!MmIsAddressValidEx(g_fnRZwQueryVirtualMemory)){
			g_fnRZwQueryVirtualMemory = GetZwQueryVirtualMemoryAddress();
...
VOID EunmProcessModule(ULONG Eprocess,PDLLINFO PDll)
{
	...
	if (!MmIsAddressValidEx((PVOID)Eprocess) ||
		!IsExitProcess((PEPROCESS)Eprocess))
	{
		return;
	}
	if (PDll)
	{
		ModuleCount = 0;
		ProcessFullPathFromVAD((PEPROCESS)Eprocess,PDll);
		if (!PDll->ulCount)
		{
