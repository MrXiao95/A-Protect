...
VOID SetNsiproxyHook(ULONG ulNumber,ULONG ulRealDispatch)
{
	...
	if (MmIsAddressValidEx(PNsiproxyDriverObjectBakup))
	{
		...
		PNsiproxyDriverObject->MajorFunction[ulNumber] = (PDRIVER_DISPATCH)ulRealDispatch;   //恢复 fsd hook

		ulReloadDispatch = ulRealDispatch - ulNsiproxyModuleBase + ulReLoadNsiproxyModuleBase;

		//恢复 fsd inline hook
		if (!MmIsAddressValidEx((PVOID)ulReloadDispatch) ||
			GetFunctionCodeSize((PVOID)ulRealDispatch) != GetFunctionCodeSize((PVOID)ulReloadDispatch))
		{
			return;
		}
...
VOID FixFixNsiproxy(PNSIPROXYDISPATCHBAKUP NsiproxyDispatchBakUp,PDRIVER_OBJECT PNsiproxyDriverObject,int i,ULONG ulReal_Dispatch,WCHAR *lpwzDispatchName,ULONG Dispatch)
{
	...
	ulCurrentNsiproxyDispatch =(ULONG) PNsiproxyDriverObject->MajorFunction[Dispatch];
	...
	if (ulCurrentNsiproxyDispatch == ulReal_Dispatch)
	{
		...
		//检查是否inline hook
		if (GetFunctionCodeSize((PVOID)ulReal_Dispatch) == GetFunctionCodeSize((PVOID)ulReloadNsiproxyDispatch) &&
			memcmp((PVOID)ulReal_Dispatch,(PVOID)ulReloadNsiproxyDispatch,GetFunctionCodeSize((PVOID)ulReal_Dispatch)) != 0)
		{
			...
			if (!MmIsAddressValidEx((PVOID)ulCurrentNsiproxyDispatch))
			{
...
NTSTATUS ReLoadNsiproxy(PDRIVER_OBJECT DriverObject,PNSIPROXYDISPATCHBAKUP NsiproxyDispatchBakUp,int Type)
{
	...
	if (GetDriverObject(L"\\Driver\\nsiproxy",&PNsiproxyDriverObject) == STATUS_SUCCESS)
	{
		...
		ulNsiproxyModuleBase =(ULONG) PNsiproxyDriverObject->DriverStart;
		ulNsiproxyModuleSize = PNsiproxyDriverObject->DriverSize;

		//reload
		if (PeLoad(
			L"\\SystemRoot\\system32\\drivers\\Nsiproxy.sys",
			(BYTE**)(&ulReLoadNsiproxyModuleBase),
			DriverObject,
			ulNsiproxyModuleBase
			))
		{
			if (GetDriverEntryPoint((PVOID)ulReLoadNsiproxyModuleBase,(DWORD*)(&NsiproxyDriverEntry)))
			{
				...
				8720f082 cc              int     3
				*/
				for (i=NsiproxyDriverEntry;i < NsiproxyDriverEntry+0x1000;i++)
				{
					if (*i == 0xe9)
					{
				...
				if (bInit)
				{
					...
					*/
					for (i=ulJmpAddress;i < ulJmpAddress+0x1000;i++)
					{
						if (*i == 0xe8)
						{
							ulAddress = (PUCHAR)(*(PULONG)(i+1)+(ULONG)(i+5));
				...
				//所有调用都走reload
				PNsiproxyDriverObjectBakup->MajorFunction[IRP_MJ_CREATE] = (PDRIVER_DISPATCH)(ulReal_NSIPROXY_IRP_MJ_CREATE - ulNsiproxyModuleBase + ulReLoadNsiproxyModuleBase);
				PNsiproxyDriverObjectBakup->MajorFunction[IRP_MJ_CREATE_NAMED_PIPE] = (PDRIVER_DISPATCH)(ulReal_NSIPROXY_IRP_MJ_CREATE_NAMED_PIPE - ulNsiproxyModuleBase + ulReLoadNsiproxyModuleBase);
				PNsiproxyDriverObjectBakup->MajorFunction[IRP_MJ_CLOSE] = (PDRIVER_DISPATCH)(ulReal_NSIPROXY_IRP_MJ_CLOSE - ulNsiproxyModuleBase + ulReLoadNsiproxyModuleBase);
				PNsiproxyDriverObjectBakup->MajorFunction[IRP_MJ_READ] = (PDRIVER_DISPATCH)(ulReal_NSIPROXY_IRP_MJ_READ - ulNsiproxyModuleBase + ulReLoadNsiproxyModuleBase);
				PNsiproxyDriverObjectBakup->MajorFunction[IRP_MJ_WRITE] = (PDRIVER_DISPATCH)(ulReal_NSIPROXY_IRP_MJ_WRITE - ulNsiproxyModuleBase + ulReLoadNsiproxyModuleBase);
				PNsiproxyDriverObjectBakup->MajorFunction[IRP_MJ_QUERY_INFORMATION] = (PDRIVER_DISPATCH)(ulReal_NSIPROXY_IRP_MJ_QUERY_INFORMATION - ulNsiproxyModuleBase + ulReLoadNsiproxyModuleBase);
				PNsiproxyDriverObjectBakup->MajorFunction[IRP_MJ_SET_INFORMATION] = (PDRIVER_DISPATCH)(ulReal_NSIPROXY_IRP_MJ_SET_INFORMATION - ulNsiproxyModuleBase + ulReLoadNsiproxyModuleBase);
				PNsiproxyDriverObjectBakup->MajorFunction[IRP_MJ_QUERY_EA] = (PDRIVER_DISPATCH)(ulReal_NSIPROXY_IRP_MJ_QUERY_EA - ulNsiproxyModuleBase + ulReLoadNsiproxyModuleBase);
				PNsiproxyDriverObjectBakup->MajorFunction[IRP_MJ_SET_EA] = (PDRIVER_DISPATCH)(ulReal_NSIPROXY_IRP_MJ_SET_EA - ulNsiproxyModuleBase + ulReLoadNsiproxyModuleBase);
				PNsiproxyDriverObjectBakup->MajorFunction[IRP_MJ_FLUSH_BUFFERS] =(PDRIVER_DISPATCH)( ulReal_NSIPROXY_IRP_MJ_FLUSH_BUFFERS - ulNsiproxyModuleBase + ulReLoadNsiproxyModuleBase);
				PNsiproxyDriverObjectBakup->MajorFunction[IRP_MJ_QUERY_VOLUME_INFORMATION] = (PDRIVER_DISPATCH)(ulReal_NSIPROXY_IRP_MJ_QUERY_VOLUME_INFORMATION - ulNsiproxyModuleBase + ulReLoadNsiproxyModuleBase);
				PNsiproxyDriverObjectBakup->MajorFunction[IRP_MJ_SET_VOLUME_INFORMATION] =(PDRIVER_DISPATCH)( ulReal_NSIPROXY_IRP_MJ_SET_VOLUME_INFORMATION - ulNsiproxyModuleBase + ulReLoadNsiproxyModuleBase);
				PNsiproxyDriverObjectBakup->MajorFunction[IRP_MJ_DIRECTORY_CONTROL] =(PDRIVER_DISPATCH)( ulReal_NSIPROXY_IRP_MJ_DIRECTORY_CONTROL - ulNsiproxyModuleBase + ulReLoadNsiproxyModuleBase);
				PNsiproxyDriverObjectBakup->MajorFunction[IRP_MJ_FILE_SYSTEM_CONTROL] =(PDRIVER_DISPATCH)( ulReal_NSIPROXY_IRP_MJ_FILE_SYSTEM_CONTROL - ulNsiproxyModuleBase + ulReLoadNsiproxyModuleBase);
				PNsiproxyDriverObjectBakup->MajorFunction[IRP_MJ_DEVICE_CONTROL] = (PDRIVER_DISPATCH)(ulReal_NSIPROXY_IRP_MJ_DEVICE_CONTROL - ulNsiproxyModuleBase + ulReLoadNsiproxyModuleBase);
				PNsiproxyDriverObjectBakup->MajorFunction[IRP_MJ_SHUTDOWN] =(PDRIVER_DISPATCH)( ulReal_NSIPROXY_IRP_MJ_SHUTDOWN - ulNsiproxyModuleBase + ulReLoadNsiproxyModuleBase);
				PNsiproxyDriverObjectBakup->MajorFunction[IRP_MJ_LOCK_CONTROL] = (PDRIVER_DISPATCH)(ulReal_NSIPROXY_IRP_MJ_LOCK_CONTROL - ulNsiproxyModuleBase + ulReLoadNsiproxyModuleBase);
				PNsiproxyDriverObjectBakup->MajorFunction[IRP_MJ_CLEANUP] = (PDRIVER_DISPATCH)(ulReal_NSIPROXY_IRP_MJ_CLEANUP - ulNsiproxyModuleBase + ulReLoadNsiproxyModuleBase);
				PNsiproxyDriverObjectBakup->MajorFunction[IRP_MJ_CREATE_MAILSLOT] = (PDRIVER_DISPATCH)(ulReal_NSIPROXY_IRP_MJ_CREATE_MAILSLOT - ulNsiproxyModuleBase + ulReLoadNsiproxyModuleBase);
				PNsiproxyDriverObjectBakup->MajorFunction[IRP_MJ_QUERY_SECURITY] =(PDRIVER_DISPATCH)( ulReal_NSIPROXY_IRP_MJ_QUERY_SECURITY - ulNsiproxyModuleBase + ulReLoadNsiproxyModuleBase);
				PNsiproxyDriverObjectBakup->MajorFunction[IRP_MJ_SET_SECURITY] =(PDRIVER_DISPATCH)( ulReal_NSIPROXY_IRP_MJ_SET_SECURITY - ulNsiproxyModuleBase + ulReLoadNsiproxyModuleBase);
				PNsiproxyDriverObjectBakup->MajorFunction[IRP_MJ_POWER] =(PDRIVER_DISPATCH)( ulReal_NSIPROXY_IRP_MJ_POWER - ulNsiproxyModuleBase + ulReLoadNsiproxyModuleBase);
				PNsiproxyDriverObjectBakup->MajorFunction[IRP_MJ_SYSTEM_CONTROL] = (PDRIVER_DISPATCH)(ulReal_NSIPROXY_IRP_MJ_SYSTEM_CONTROL - ulNsiproxyModuleBase + ulReLoadNsiproxyModuleBase);
				PNsiproxyDriverObjectBakup->MajorFunction[IRP_MJ_DEVICE_CHANGE] =(PDRIVER_DISPATCH)( ulReal_NSIPROXY_IRP_MJ_DEVICE_CHANGE - ulNsiproxyModuleBase + ulReLoadNsiproxyModuleBase);
				PNsiproxyDriverObjectBakup->MajorFunction[IRP_MJ_QUERY_QUOTA] = (PDRIVER_DISPATCH)(ulReal_NSIPROXY_IRP_MJ_QUERY_QUOTA - ulNsiproxyModuleBase + ulReLoadNsiproxyModuleBase);
				PNsiproxyDriverObjectBakup->MajorFunction[IRP_MJ_SET_QUOTA] = (PDRIVER_DISPATCH)(ulReal_NSIPROXY_IRP_MJ_SET_QUOTA - ulNsiproxyModuleBase + ulReLoadNsiproxyModuleBase);
				PNsiproxyDriverObjectBakup->MajorFunction[IRP_MJ_PNP_POWER] = (PDRIVER_DISPATCH)(ulReal_NSIPROXY_IRP_MJ_PNP_POWER - ulNsiproxyModuleBase + ulReLoadNsiproxyModuleBase);
			}
		}
	}
	return STATUS_SUCCESS;
...
NTSTATUS ReLoadNsiproxyFree()
{
	...
	if (MmIsAddressValidEx(PNsiproxyDriverObjectBakup))
	{
		...
		{
			PNsiproxyDriverObjectBakup->MajorFunction[IRP_MJ_CREATE] =(PDRIVER_DISPATCH) ulReal_NSIPROXY_IRP_MJ_CREATE;
			PNsiproxyDriverObjectBakup->MajorFunction[IRP_MJ_CREATE_NAMED_PIPE] = (PDRIVER_DISPATCH)ulReal_NSIPROXY_IRP_MJ_CREATE_NAMED_PIPE;
			PNsiproxyDriverObjectBakup->MajorFunction[IRP_MJ_CLOSE] =(PDRIVER_DISPATCH) ulReal_NSIPROXY_IRP_MJ_CLOSE;
			PNsiproxyDriverObjectBakup->MajorFunction[IRP_MJ_READ] =(PDRIVER_DISPATCH) ulReal_NSIPROXY_IRP_MJ_READ;
			PNsiproxyDriverObjectBakup->MajorFunction[IRP_MJ_WRITE] =(PDRIVER_DISPATCH) ulReal_NSIPROXY_IRP_MJ_WRITE;
			PNsiproxyDriverObjectBakup->MajorFunction[IRP_MJ_QUERY_INFORMATION] = (PDRIVER_DISPATCH)ulReal_NSIPROXY_IRP_MJ_QUERY_INFORMATION;
			PNsiproxyDriverObjectBakup->MajorFunction[IRP_MJ_SET_INFORMATION] =(PDRIVER_DISPATCH) ulReal_NSIPROXY_IRP_MJ_SET_INFORMATION;
			PNsiproxyDriverObjectBakup->MajorFunction[IRP_MJ_QUERY_EA] =(PDRIVER_DISPATCH) ulReal_NSIPROXY_IRP_MJ_QUERY_EA;
			PNsiproxyDriverObjectBakup->MajorFunction[IRP_MJ_SET_EA] = (PDRIVER_DISPATCH)ulReal_NSIPROXY_IRP_MJ_SET_EA;
			PNsiproxyDriverObjectBakup->MajorFunction[IRP_MJ_FLUSH_BUFFERS] = (PDRIVER_DISPATCH)ulReal_NSIPROXY_IRP_MJ_FLUSH_BUFFERS;
			PNsiproxyDriverObjectBakup->MajorFunction[IRP_MJ_QUERY_VOLUME_INFORMATION] =(PDRIVER_DISPATCH) ulReal_NSIPROXY_IRP_MJ_QUERY_VOLUME_INFORMATION;
			PNsiproxyDriverObjectBakup->MajorFunction[IRP_MJ_SET_VOLUME_INFORMATION] = (PDRIVER_DISPATCH)ulReal_NSIPROXY_IRP_MJ_SET_VOLUME_INFORMATION;
			PNsiproxyDriverObjectBakup->MajorFunction[IRP_MJ_DIRECTORY_CONTROL] = (PDRIVER_DISPATCH)ulReal_NSIPROXY_IRP_MJ_DIRECTORY_CONTROL;
			PNsiproxyDriverObjectBakup->MajorFunction[IRP_MJ_FILE_SYSTEM_CONTROL] =(PDRIVER_DISPATCH) ulReal_NSIPROXY_IRP_MJ_FILE_SYSTEM_CONTROL;
			PNsiproxyDriverObjectBakup->MajorFunction[IRP_MJ_DEVICE_CONTROL] = (PDRIVER_DISPATCH)ulReal_NSIPROXY_IRP_MJ_DEVICE_CONTROL;
			PNsiproxyDriverObjectBakup->MajorFunction[IRP_MJ_SHUTDOWN] =(PDRIVER_DISPATCH) ulReal_NSIPROXY_IRP_MJ_SHUTDOWN;
			PNsiproxyDriverObjectBakup->MajorFunction[IRP_MJ_LOCK_CONTROL] = (PDRIVER_DISPATCH)ulReal_NSIPROXY_IRP_MJ_LOCK_CONTROL;
			PNsiproxyDriverObjectBakup->MajorFunction[IRP_MJ_CLEANUP] = (PDRIVER_DISPATCH)ulReal_NSIPROXY_IRP_MJ_CLEANUP;
			PNsiproxyDriverObjectBakup->MajorFunction[IRP_MJ_CREATE_MAILSLOT] =(PDRIVER_DISPATCH) ulReal_NSIPROXY_IRP_MJ_CREATE_MAILSLOT;
			PNsiproxyDriverObjectBakup->MajorFunction[IRP_MJ_QUERY_SECURITY] = (PDRIVER_DISPATCH)ulReal_NSIPROXY_IRP_MJ_QUERY_SECURITY;
			PNsiproxyDriverObjectBakup->MajorFunction[IRP_MJ_SET_SECURITY] =(PDRIVER_DISPATCH) ulReal_NSIPROXY_IRP_MJ_SET_SECURITY;
			PNsiproxyDriverObjectBakup->MajorFunction[IRP_MJ_POWER] =(PDRIVER_DISPATCH) ulReal_NSIPROXY_IRP_MJ_POWER;
			PNsiproxyDriverObjectBakup->MajorFunction[IRP_MJ_SYSTEM_CONTROL] =(PDRIVER_DISPATCH) ulReal_NSIPROXY_IRP_MJ_SYSTEM_CONTROL;
			PNsiproxyDriverObjectBakup->MajorFunction[IRP_MJ_DEVICE_CHANGE] = (PDRIVER_DISPATCH)ulReal_NSIPROXY_IRP_MJ_DEVICE_CHANGE;
			PNsiproxyDriverObjectBakup->MajorFunction[IRP_MJ_QUERY_QUOTA] = (PDRIVER_DISPATCH)ulReal_NSIPROXY_IRP_MJ_QUERY_QUOTA;
			PNsiproxyDriverObjectBakup->MajorFunction[IRP_MJ_SET_QUOTA] = (PDRIVER_DISPATCH)ulReal_NSIPROXY_IRP_MJ_SET_QUOTA;
			PNsiproxyDriverObjectBakup->MajorFunction[IRP_MJ_PNP_POWER] = (PDRIVER_DISPATCH)ulReal_NSIPROXY_IRP_MJ_PNP_POWER;
		}
	}
	return STATUS_SUCCESS;
}