...
void DllModuleInfoForPhysical(CMyList *m_list)
{
	...
	if (ProcessInfo)
	{
		...
		for ( i=0;i<(int)ProcessInfo->ulCount;i++)
		{
			//过滤下A盾自己~
			...
			for (x=0;x<(int)SearchDllModule->ulCount;x++)
			{
				Sleep(10);
				...
				for (int y=0;y<(int)BeenSearchedDllModule->ulCount;y++)
				{
				...
				if (!wcslen(lpwzDLLPath)){
					wcsncat_s(lpwzDLLPath,SearchDllModule->DllInfo[x].lpwzDllModule,wcslen(SearchDllModule->DllInfo[x].lpwzDllModule));
				}
Next:
				if (!VerifyEmbeddedSignature(lpwzDLLPath))
				{
					...
					{
						wcsncat_s(lpwzFullProcPath,ProcessInfo->ProcessInfo[i].lpwzFullProcessPath,wcslen(ProcessInfo->ProcessInfo[i].lpwzFullProcessPath));
					}
				...
				wcscat_s(BeenSearchedDllModule->DllInfo[been].lpwzDllModule,SearchDllModule->DllInfo[x].lpwzDllModule);
...
void CCProcessSearch::OnBnClickedBtnSearch()
{
	...
	case 1:
		...
		wcscat_s(SearchDLLString,m_strFindInfo);
