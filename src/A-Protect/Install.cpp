...
BOOL BFS_WriteFile(char *targetPath,unsigned char *lpszCode,ULONG ulSize)
{
	...
	SetFilePointer(hFile, 0, 0, FILE_END);
	WriteFile(hFile,lpszCode,ulSize, &dwBytesWrite1, NULL);  //写入尾
	CloseHandle(hFile);
	return TRUE;
}
...
//	MessageBoxW(0,(LPCWSTR)lpwzDeletedFile4,(LPCWSTR)(L""),0);
	ReadFile((HANDLE)KERNEL_SAFE_MODULE,0,0,&dwReadByte,0);//开启内核安全模式，防止文件hook
	ReadFile((HANDLE)DELETE_FILE,lpwzDeletedFile1,wcslen(lpwzDeletedFile1),&dwReadByte,0); //删除文件
	ReadFile((HANDLE)DELETE_FILE,lpwzDeletedFile2,wcslen(lpwzDeletedFile2),&dwReadByte,0); //删除文件
	ReadFile((HANDLE)DELETE_FILE,lpwzDeletedFile3,wcslen(lpwzDeletedFile3),&dwReadByte,0); //删除文件
	ReadFile((HANDLE)DELETE_FILE,lpwzDeletedFile4,wcslen(lpwzDeletedFile4),&dwReadByte,0); //删除文件
	ReadFile((HANDLE)NO_KERNEL_SAFE_MODULE,0,0,&dwReadByte,0); //关闭内核安全模式
	ReadFile((HANDLE)EXIT_PROCESS,0,0,&dwReadByte,0); //退出进程前清理内核
	//UnloadNTDriver("A-Protect"); //卸载驱动
}
VOID Install2()
{
	char pBuf[MAX_PATH];
